...
void Library<T,U>::add(const T& item) {
	...
	if(find_if(items.cbegin(), items.cend(), [&item](auto it){
...
unsigned int Library<T,U>::remove(const T& item) {
	...
	auto rm = std::find_if(items.cbegin(), items.cend(), [&item](auto it){
		return it.getItem() == item;
	});
	if(rm != items.cend())items.erase(rm);
	return items.size();
}
...
void Library<T,U>::ItemContainer::RelatedWork::printItemAndDescription() {
	if(relatedWorkLink.lock())
		std::cout << *(relatedWorkLink.lock()) << " - " << relatedWorkDescription << std::endl;
}
